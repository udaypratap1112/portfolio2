...
const progress = useTransform(scrollYProgress, [0, 1], [0, 100]);
  ...
  const scrollX = size>640?useTransform(springProgress, [0,40,80,100], [0,parseInt(`-${stops.left[0]}`) ,parseInt(`-${stops.left[0]}`),parseInt(`-${stops.left[1]}`)]):0;
  const scrolllY = size > 640 ? 0 : useTransform(springProgress, [0,40,80,100], [0,parseInt(`-${stops.top[0]}`) ,parseInt(`-${stops.top[0]}`),parseInt(`-${stops.top[1]}`)]);
  ...
  return (
    <div ref={container} style={containerStyle} className="hero overflow-y-auto overflow-x-hidden " >
      <div className={"h-[1000vh] box-border relative "}>
        <div className="sticky left-0 top-0 w-full h-screen bg-[var(--silk)] dark:bg-bg-dark  overflow-hidden">
      <Navbar stops={stops}></Navbar>
        <motion.div className="content bg-inherit flex  flex-col sm:flex-row " style={{ x: scrollX,y:scrolllY }}>
